...
# • Emoji’s in de 4 eigenschapsregels vervangen door Heroicons (SVG) via CDN-injectie.
# • Prompt en fallback-body produceren geen emoji meer; iconen worden server-side toegevoegd.
# • Env: HEROICON_CDN (default jsDelivr @latest), HEROICON_SIZE (default 20).
...
# Heroicons configuratie
HEROICON_CDN  = os.environ.get("HEROICON_CDN", "https://cdn.jsdelivr.net/npm/heroicons@latest/24/outline").rstrip("/")
...
def split_ai_output(text:str)->Dict[str,str]:
    ...
    # Fallback body zonder emoji; iconen worden later geïnjecteerd
...
# ---------- Heroicons injectie (NIEUW) ----------

def _icon_img(name:str)->str:
    return (f'<img src="{HEROICON_CDN}/{name}.svg" alt="" width="{HEROICON_SIZE}" height="{HEROICON_SIZE}" '
            'style="vertical-align:text-bottom;margin-right:6px;opacity:.9" loading="lazy" />')

ICON_MAP = {
    "licht": _icon_img("sun"),
    "water": _icon_img("droplet"),
    "plaats": _icon_img("home"),
    "giftig": _icon_img("exclamation-triangle"),
}

def inject_heroicons(body_html:str)->str:
    """Vervang emoji/geen-icon varianten door Heroicons vóór de 4 regels. Idempotent."""
    if not body_html: return body_html
    if "heroicons" in body_html or 'exclamation-triangle' in body_html or 'sun.svg' in body_html:
        return body_html  # al gebeurd
    out = body_html

    patterns = [
        (re.compile(r"(<p[^>]*>\s*)(?:[☀]\s*)?(?:<strong>)?Lichtbehoefte(?:</strong>)?\s*:\s*(.*?)(</p>)", re.I|re.S),
         f"{ICON_MAP['licht']}<strong>Lichtbehoefte:</strong> "),
        (re.compile(r"(<p[^>]*>\s*)(?:[∿~]\s*)?(?:<strong>)?Waterbehoefte(?:</strong>)?\s*:\s*(.*?)(</p>)", re.I|re.S),
         f"{ICON_MAP['water']}<strong>Waterbehoefte:</strong> "),
        (re.compile(r"(<p[^>]*>\s*)(?:[⌂]\s*)?(?:<strong>)?Standplaats(?:</strong>)?\s*:\s*(.*?)(</p>)", re.I|re.S),
         f"{ICON_MAP['plaats']}<strong>Standplaats:</strong> "),
        (re.compile(r"(<p[^>]*>\s*)(?:[☠]\s*)?(?:<strong>)?Giftigheid(?:</strong>)?\s*:\s*(.*?)(</p>)", re.I|re.S),
         f"{ICON_MAP['giftig']}<strong>Giftigheid:</strong> "),
    ]

    for rx, repl_prefix in patterns:
        out = rx.sub(lambda m: f"{m.group(1)}{repl_prefix}{m.group(2)}{m.group(3)}", out)
    return out
...
def _rank_candidates(defs:List[Dict[str,Any]], hints:List[str])->List[Dict[str,Any]]:
    ...
    for d in defs:
        ...
        out.append({"namespace":ns,"key":key,"name":name,"type":(d.get("type") or {}).get("name") or "single_line_text_field","score":score})
...
def _set_one(token:str, store_domain:str, gid:str, ns:str, key:str, tname:str, value:str)->Tuple[bool,str]:
    ...
    payload={"query":mutation,"variables":{"metafields":[{"ownerId":gid,"namespace":ns,"key":key,"type":tname,"value":_encode_value(value,tname)}]}}
            ...
            for i in range(0, len(all_pids), BATCH_SIZE):
                ...
                for p in prods:
                    ...
                    try:
                        ...
                        # Heroicons aan de 4 regels toevoegen
